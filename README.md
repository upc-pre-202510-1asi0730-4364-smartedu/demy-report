...
| 40      | TS06          | Implementar endpoint para obtener estudiante por ID (`GET /students/{id}`)                                  | Como developer, quiero exponer un endpoint que devuelva un estudiante por su ID, para que los usuarios puedan visualizar sus datos personales.                                                  | **2**        |
| 41      | TS07          | Implementar endpoint para listar estudiantes (`GET /students`)                                              | Como developer, quiero exponer un endpoint que devuelva el listado de estudiantes, para que los usuarios puedan consultar todos los registros de estudiantes.                                   | **2**        |
| 42      | TS09          | Implementar endpoint para obtener período académico por ID (`GET /academic-periods/{id}`)                   | Como developer, quiero exponer un endpoint que devuelva un período académico por su ID, para ver sus fechas y estado.                                                                           | **2**        |
| 43      | TS10          | Implementar endpoint para listar períodos académicos (`GET /academic-periods`)                              | Como developer, quiero exponer un endpoint que devuelva el listado de períodos académicos, para que los usuarios puedan consultar todas las ventanas de matrícula definidas.                    | **2**        |
