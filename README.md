...
| 35      | TS05          | Implementar endpoint para registrar y gestionar estudiantes (`POST, PUT, DELETE /students`)                 | Como developer, quiero exponer endpoints que gestione un estudiante, para registrar, actualizar o eliminar su información en el sistema.                                                        | **5**        |
| 36      | TS08          | Implementar endpoint para registrar y gestionar periodos académicos (`POST, PUT, DELETE /academic-periods`) | Como developer, quiero exponer endpoints que gestionen un período académico, para registrar, actualizar o eliminar su información en el sistema.                                                | **5**        |
| 37      | TS04          | Implementar endpoint para consultar matrícula por DNI (`GET /enrollments/dni/{dni}`)                        | Como developer, quiero exponer un endpoint que devuelva una matrícula según el DNI del estudiante, para que los administradores puedan consultar la matrícula sin conocer su ID interno.        | **3**        |
| 38      | TS02          | Implementar endpoint para consultar matrícula por ID (`GET /enrollments/{id}`)                              | Como developer, quiero exponer un endpoint que devuelva una matrícula por su ID, para que los administradores puedan obtener sus detalles.                                                      | **2**        |
| 39      | TS03          | Implementar endpoint para listar matrículas (`GET /enrollments`)                                            | Como developer, quiero exponer un endpoint que devuelva el listado de matrículas, para que los administradores puedan consultar todas las matrículas registradas.                               | **2**        |
| 40      | TS06          | Obtener estudiante por ID (`GET /students/{id}`)                                                            | Como developer, quiero exponer un endpoint que devuelva un estudiante por su ID, para que los usuarios puedan visualizar sus datos personales.                                                  | **2**        |
| 41      | TS07          | Listar estudiantes (`GET /students`)                                                                        | Como developer, quiero exponer un endpoint que devuelva el listado de estudiantes, para que los usuarios puedan consultar todos los registros de estudiantes.                                   | **2**        |
| 42      | TS09          | Obtener período académico por ID (`GET /academic-periods/{id}`)                                             | Como developer, quiero exponer un endpoint que devuelva un período académico por su ID, para ver sus fechas y estado.                                                                           | **2**        |
| 43      | TS10          | Listar períodos académicos (`GET /academic-periods`)                                                        | Como developer, quiero exponer un endpoint que devuelva el listado de períodos académicos, para que los usuarios puedan consultar todas las ventanas de matrícula definidas.                    | **2**        |
